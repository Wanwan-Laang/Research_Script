...
def plot_single(fig_name, x, y, xlabel, ylabel, title, color):
    ...
    #plt.title(title)
    plt.grid(False)
    plt.savefig(fig_name, dpi=900, bbox_inches='tight', transparent=True)  # 添加 transparent=True
    plt.close()

def plot_step_log_evolution(steps, data_map, out_png, color_map):
    fig, axs = plt.subplots(2, 4, figsize=(24, 9), constrained_layout=True)
    keys = list(data_map.keys())
    for i, key in enumerate(keys):
        row, col = divmod(i, 4)
        y, ylabel = data_map[key]
        axs[row][col].plot(steps, y, color=color_map[key], linewidth=3)
        axs[row][col].set_xlim(steps[0], steps[-1])
        axs[row][col].set_xscale('log')
        axs[row][col].set_xlabel("Step (log)")
        axs[row][col].set_ylabel(ylabel)
        axs[row][col].grid(False)
    plt.suptitle("Log-Step Evolution from LAMMPS log")
    plt.savefig(out_png, dpi=900, bbox_inches='tight', transparent=True)  # 添加 transparent=True
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    data_map = {
        ...
        "vol":   (vols, "Volume"),
        "dens":  (dens, "Density (g/cm^3)"),
        "pe":    (pes, "Potential Energy (eV)"),
        "ke":    (kes, "Kinetic Energy (eV)"),
        "etot":  (etots, "Total Energy (eV)")
    }
    ...
    for i, key in enumerate(keys_to_plot):
        ...
        axs[row][col].set_xlim(x[0], x[-1])
        axs[row][col].set_xlabel(x_label)
        axs[row][col].set_ylabel(ylabel)
        axs[row][col].grid(False)
    plt.suptitle("Time Evolution from LAMMPS log")
    plt.savefig(args.output, dpi=900, bbox_inches='tight', transparent=True)  # 添加 transparent=True
    plt.close(fig)
