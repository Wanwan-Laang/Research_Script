...
def compute_time_series_stats(df, prefix):
    """Compute frame-by-frame cluster count and save to CSV."""
    stats = df.groupby('frame').agg({
        'cluster_id': 'count'
    }).reset_index()
    stats.columns = ['frame', 'cluster_count']
    out_csv = f"cluster_count_summary_{prefix}.csv"
    stats.to_csv(out_csv, index=False)
    print(f"Saved time series stats: {out_csv}")
    return stats

def plot_time_series(stats, prefix):
    """Plot cluster count over frames and save as pdf."""
    fig, ax = plt.subplots(figsize=(8,5))
    ax.plot(stats['frame'], stats['cluster_count'], '-o')
    ax.set_xlabel('Frame')
    ax.set_ylabel('Cluster Count')
    plt.xlim(stats['frame'].min(), stats['frame'].max())
    plt.tight_layout()
    out_pdf = f"cluster_count_time_series_{prefix}.pdf"
